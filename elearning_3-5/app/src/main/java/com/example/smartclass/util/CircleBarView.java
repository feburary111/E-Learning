...
public class CircleBarView extends View {
    ...
    private void init(Context context,AttributeSet attrs){
        ...
        barWidth = typedArray.getDimension(R.styleable.CircleBarView_bar_width,MiscUtil.dipToPx(context,10));
        typedArray.recycle();

        progressNum = 0;
        maxNum = 100;
        defaultSize = MiscUtil.dipToPx(context,100);
        mRectF = new RectF();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float outerRadius = mRectF.width() + progressPaint.getStrokeWidth();
        float offsetX = (getWidth() - outerRadius * 2) / 2;
        float offsetY = (getHeight() - outerRadius * 2) / 2;

        LinearGradient shader = new LinearGradient(0, offsetY, 0, offsetY + outerRadius * 2, colorArray,
